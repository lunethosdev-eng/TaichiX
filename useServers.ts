  ...
  const startServer = useCallback(
    async (id: string) => {
      ...
      try {
        ...
        let avail;
        try {
          avail = await ServerNative.isAvailable();
        } catch (e: any) {
          const msg = String(e?.message || e);
          if (msg.includes('not implemented')) {
            await appendLogs(id, [
              'ERROR: El plugin nativo NO está en este APK.',
              'Sube el workflow nuevo (build-apk.yml) y el ServerNativePlugin.java,',
              'vuelve a construir el APK en GitHub Actions e instálalo.',
            ]);
        ...
        if (server.type === 'java' && !avail?.javaRuntime) {
          await appendLogs(id, [
            'ERROR: No hay Java (JRE) en el teléfono.',
            'Para un servidor Paper REAL hace falta un JRE arm64 en:',
            '  Android/data/.../files/runtime/java/bin/java',
            'O embebido en la próxima versión del APK.',
            'Sin Java el sistema Android no puede ejecutar el .jar.',
          ]);
          await setStatus(id, 'error');
          return;
        }
