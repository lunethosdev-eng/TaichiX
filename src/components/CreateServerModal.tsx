...
import { X, Server, Cpu, Package } from 'lucide-react';
import type { CreateServerForm, DeviceInfo, ServerType, ModLoader } from '../types/server';
...
interface Props {
  open: boolean;
  onClose: () => void;
  onCreate: (form: CreateServerForm) => Promise<unknown>;
...
export function CreateServerModal({ open, onClose, onCreate, deviceInfo }: Props) {
  ...
  const handleTypeChange = (type: ServerType) => {
    setForm(f => ({
      ...
      loader: type === 'java' ? 'paper' : undefined,
      selectedMods: [],
    }));
  };
  ...
  const modCount = form.selectedMods?.length ?? 0;
  ...
  return (
    <>
      <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4 bg-black/70 backdrop-blur-sm">
        <Card className="w-full max-w-lg max-h-[92vh] overflow-y-auto rounded-b-none sm:rounded-xl">
          ...
          <form onSubmit={handleSubmit} className="p-4 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <select
                  value={form.version}
                  onChange={e =>
                    setForm(f => ({ ...f, version: e.target.value, selectedMods: [] }))
                  }
                  className="w-full px-3 py-2 rounded-lg bg-slate-900 border border-slate-600 focus:border-emerald-500 focus:outline-none"
                >
                  {versions.map(v => (
            ...
            {/* Tienda de mods / addons */}
            <button
              type="button"
              onClick={() => setShowMods(true)}
              className="w-full flex items-center justify-between p-3 rounded-lg border border-slate-600 hover:border-emerald-500 text-left transition-colors"
            >
              <span className="flex items-center gap-2 text-sm">
                <Package className="w-4 h-4 text-emerald-400" />
                Tienda de {form.type === 'java' ? 'mods' : 'addons'}
              </span>
              <span className="text-xs text-slate-400">
                {modCount === 0 ? 'Ninguno' : `${modCount} seleccionado${modCount > 1 ? 's' : ''}`}
              </span>
            </button>
      ...
      <ModStore
        ...
        loader={(form.loader as ModLoader) || 'paper'}
        onLoaderChange={loader => setForm(f => ({ ...f, loader, selectedMods: [] }))}
        selectedIds={form.selectedMods || []}
        onChangeSelected={ids => setForm(f => ({ ...f, selectedMods: ids }))}
      />
    </>
  );
}
