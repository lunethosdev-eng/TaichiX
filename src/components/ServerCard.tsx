import { Play, Square, Trash2, Users, Cpu, Copy, Check, Terminal, Globe, Wifi, Package } from 'lucide-react';
...
export function ServerCard({ server, onStart, onStop, onDelete, onOpenConsole }: Props) {
  ...
  const publicFull = server.publicAddress
    ? `${server.publicAddress}${
        server.publicPort && server.publicPort !== 25565 && server.publicPort !== 19132
          ? ':' + server.publicPort
          : ''
      }`
    : '';
    ...
    server.status === 'running' && server.publicAddress
      ? `¡Únete a mi servidor de Minecraft!\n\n` +
        ...
        (server.installedMods && server.installedMods.length > 0
          ? `Mods: ${server.installedMods.map(m => m.name).join(', ')}\n`
          : '') +
        `\nSolo pega la dirección en Minecraft y listo.`
      : '';

  const modCount = server.installedMods?.length ?? 0;
              ...
              {modCount > 0 && (
                <Badge color="purple">
                  <span className="inline-flex items-center gap-0.5">
                    <Package className="w-3 h-3" /> {modCount} mod{modCount > 1 ? 's' : ''}
                  </span>
                </Badge>
              )}
            </div>
        ...
        {server.status === 'running' && server.publicAddress && (
          <div className="mt-3 p-3 rounded-lg bg-emerald-950/40 border border-emerald-700/50">
            ...
            <div className="flex items-center gap-2">
              ...
              <Button size="sm" variant="secondary" onClick={() => copyText(publicFull, 'public')}>
                {copied === 'public' ? (
            ...
            <div className="mt-2 flex gap-2">
              <Button size="sm" className="flex-1" onClick={() => copyText(inviteMessage, 'code')}>
                {copied === 'code' ? (
                  <>
